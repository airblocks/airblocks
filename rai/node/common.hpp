...
public:
	...
	static std::array<uint8_t, 2> constexpr magic_number = rai::rai_network == rai::rai_networks::rai_test_network ? std::array<uint8_t, 2> ({ 'A', 'A' }) : rai::rai_network == rai::rai_networks::rai_beta_network ? std::array<uint8_t, 2> ({ 'A', 'B' }) : std::array<uint8_t, 2> ({ 'A', 'C' });
	uint8_t version_max;
